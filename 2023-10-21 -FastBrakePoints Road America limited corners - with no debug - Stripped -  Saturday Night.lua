-- GPS Track Marker 
-- Dave Franchino - dave.franchino@gmail.com 
...
-- V17 strip everything I can think of to save memory 
-- V18 -- removed table for averaging GPS points and removed code that changes colors of LEDs - in the name of saving space.  

--[[
Purpose:
This program is designed to assist in improving their lap timings by providing braking information. As the driver approaches the braking point of their fastest lap, the program counts down the distance to that point using a ShiftX3 display.

How it Works:

Initialization:

The program initializes with a set refresh rate and various channels like "CORNER_DIST" and "BRAKES".
Variables for laps, distances, and brake points are defined. Flags are also initialized.
GPS coordinates for specific corners at the Road America track are listed.

Checking Proximity to Corner:
The program checks the current distance to the targeted corner. If the vehicle is within a specific distance from the corner, the target corner is incremented.

Updating Brake Points:
If the vehicle is within the brake zone (i.e., a specified distance before a corner) and the brakes have been applied, the current lap's brake point (GPS Latitiude, longitude) is recorded for that corner.

Display Update:
Depending on the distance to the fastest previous lap's brake point, the ShiftX3 display shows a countdown. For instance, as the car approaches the brake point, the display shows 3, 2, and 1, indicating how close the driver is to that point.
This countdown is represented using both numerical markers and LED indications.

Brake Check:
The program continuously monitors the brake pressure. If it exceeds a certain threshold, it registers that the brakes have been applied.

Lap Monitoring:
The program checks if the car has completed a lap. If it has, it compares the current lap's time to the best lap's time to determine if a new fastest lap has been achieved.

Key Functions:

  check_if_at_corner(): Determines which corner the car is approaching.
  update_brake_point(): Updates the brake points if the car is in the braking zone and the brakes are applied.
  update_display(): Updates the ShiftX3 display based on the distance to the brake point of the fastest lap.
  distance(): Calculates the distance between two GPS points using the Haversine formula.
  check_brakes(): Determines if the brakes have been applied.
  check_if_passed_start(): Monitors if the car has passed the starting line.
  Note: For simulation purposes and to save memory, there are sections in the code that have been commented out.
--]]
...
-- d_corner = addChannel("CORNER_DIST",10) -- this channel outputs distance as virtual channel - not used by program but monitored in race capture -- comment out to save space
...
--brake_warn = 800 -- the number of feet before the brake point that the counter will begin to decrement -- move into code to save space 
...
--]]
prev_distance = 0 -- will only use one prev-distance 
...
function update_display() -- initiates the ShiftX display based on distance to brake point for the upcoming corner for the fastest lap
  local brake_warn = 800  -- this is how far away from tbe brake point we'll begin counting down 
...
-- remove table to try to save memory... 
--[[
...
--]]
   ...
   if distance_to_fastest_brake_point >= prev_distance then   --Further away than the previous distance - moving AWAY from the fastest brake point 
      passed_brake_point = true
   end
   
   prev_distance = distance_to_fastest_brake_point  -- update the previous distance value
    ...
    if new_lap > current_lap then 
        ...
        if getLapTime() < best_lap_time then
            ...
            for i = 1, num_corners, 1 do
                ...
                --println('fl = '..tostring(new_lap)..' # '.. tostring(i)..' Lat '..tostring(brake_point_fastest_lap_lat[i])..' Lon '..tostring(brake_point_fastest_lap_lon[i])) -- comment out to save space.  
            end
        end
...
--debugging printlns.  can be commented out to save space for operation and provide space for more corners?
--[[
  println(" ")
